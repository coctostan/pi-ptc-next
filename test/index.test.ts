...
test("ptc extension bootstraps and cleans up runtime components", async () => {
  ...
  try {
    ...
    assert.ok(latestCodeExecutionTool.description.length < 250, "code_execution prompt should stay compact");
    assert.match(latestCodeExecutionTool.description, /Run Python orchestration for repo-wide or batched analysis using local tool wrappers\./);
    assert.match(latestCodeExecutionTool.description, /Use ptc\.list_helpers\(\) and ptc\.help\(name\) for available helpers\./);
    assert.match(latestCodeExecutionTool.description, /Direct wrappers include read, grep, find, ls, and glob\./);
    ...
    assert.doesNotMatch(latestCodeExecutionTool.description, /Top-level await/);
    assert.doesNotMatch(latestCodeExecutionTool.description, /Docker isolation/);
